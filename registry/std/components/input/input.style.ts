...
export const useInputStyle = createVariants({
  slots: {
    ...
    clear: [
      'relative flex-(~ center) op-0',
      'before:(abs inset-0 block content-[""] rounded-full transition-background)',
      'before:hover:(bg-current op-20)',
    ],
    counter: 'text-toned',
    suffix: '',
    append: '',
  ...
  variants: {
    ...
    size: {
      xs: { root: 'h-5 text-2.5', clear: 'mr-1 p-0.25' },
      sm: { root: 'h-6 text-2.5', clear: 'mr-1.5 p-0.5' },
      md: { root: 'h-8 text-3', clear: 'mr-2 p-0.753' },
      lg: { root: 'h-10 text-3.5', clear: 'mr-2.5 p-1' },
