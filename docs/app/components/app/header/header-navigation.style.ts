...
export const useHeaderNavigationStyle = createVariants({
  slots: {
    ...
    itemLink: [
      ...
      'c-highlighted tracking-tight hover:(c-accent-1 bg-accent)',
    ],
    itemLinkIcon: 'size-3.5 transition-colors',
    itemLinkLabel: 'text-3.5 font-medium leading-5',
    itemLinkExternalIcon: 'size-3.5 c-accent-8',
    trigger: [
      'group flex-(~ y-center) pl-4 pr-3 py-1.5 gap-2 transition-colors rounded-full',
      'c-highlighted tracking-tight hover:(c-accent-1 bg-accent) state-open:(c-accent-1 bg-accent)',
    ],
    ...
    contentItem: 'group flex-(~ y-center) gap-x-4 p-2 rounded-lg transition-colors hover:bg-accent-3',
    contentItemIcon: 'transition-colors group-hover:(bg-accent-1 shadow-xs)',
    contentItemWrapper: 'flex-(~ 1 col) gap-y-1',
    contentItemLabel: 'flex-(~ y-center) text-3.5 justify-between font-medium c-accent-12 leading-none tracking-tight',
    contentItemDescription: 'text-(xs toned)',
    contentItemExternalIcon: 'size-3.5 c-accent-8 group-hover:c-accent',
