...
<template>
  <Primitive
    ...
    @keydown="onKeyDown"
  >
    <div ref="emblaRef" :class="cn(ui.viewport(), props.ui?.viewport)">
      <div :class="cn(ui.container(), props.ui?.container)">
        <div
          v-for="(item, index) in items"
          v-bind="showDots ? { role: 'tabpanel' } : { 'role': 'group', 'aria-roledescription': 'slide' }"
          :key="index"
    ...
    <div v-if="showArrows || showDots" :class="cn(ui.controls(), props.ui?.controls)">
      <div v-if="showArrows" :class="cn(ui.arrows(), props.ui?.arrows)">
        <IconButton
      ...
      <div v-if="showDots" role="tablist" :aria-label="t('carousel.dots')" :class="cn(ui.dots(), props.ui?.dots)">
        <button
